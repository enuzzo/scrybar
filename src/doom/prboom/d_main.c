...
#ifdef SCRYBAR_PRBOOM
extern const uint8_t *scrybar_prboom_iwad_start(void);
extern size_t scrybar_prboom_iwad_size(void);
#endif
...
#ifdef SCRYBAR_PRBOOM
  if (!numwadfiles && scrybar_prboom_iwad_start() && scrybar_prboom_iwad_size() > 0) {
    wadfiles[numwadfiles++] = (wadfile_info_t){
        .name = "doom1.wad",
        .data = scrybar_prboom_iwad_start(),
        .size = scrybar_prboom_iwad_size(),
        .handle = NULL,
    };
  }
#endif
