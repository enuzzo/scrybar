...
- Decision: **Two commits.** **r275** — user-facing rename `DEPARTURES → TIMETABLE` for the LVGL header and `<strong>Departures</strong> → <strong>Timetable</strong>` for the web-UI views toggle and the Transit card heading (`Transit Departure Board → Transit Timetable`). Kept all internal identifiers (`UI_PAGE_TRANSIT`, `g_transitState`, `netFetchTransitDepartures`, `UI_VIEW_FLAG_TRANSIT`, NVS key `transit_arr`) unchanged — "transit" stays the semantic name for the Transitous data source. Added adaptive time-column formatter at [scrybar.ino:15553-15587](../scrybar.ino:15553): computes `minutes until effective departure = depHour*60+depMinute + (hasDelay?delayMin:0) - nowLocalMin`, midnight-wrap tolerance if `diff < -60` adds 1440. Display matrix: `{-1..0}` → `"now"` amber `0xFFB74D`; `{1..2}` → `"X'"` amber; `{3..60}` → `"X'"` `t.infoText`; `{>60}` → `"HH:MM"` `t.auxMeta`; `{<-1}` (past) → `"HH:MM"` `t.divider`. Prime notation chosen over `"X min"` for cross-lingua minute symbol and fit in the 70 px column at 20 px Funnel Display. Update cadence: none needed — the top-level UI render loop at [scrybar.ino:17590](../scrybar.ino:17590) already ticks every second when page is visible, so the relative string recomputes continuously. Added `[TRANSIT][PARSE]` per-row serial diagnostic (line + ISO + realTime + delay + parsed local + destination) so future data-accuracy reports can be bisected against the operator website. Helper `transitMinutesUntil(const TransitDeparture&, const struct tm&)` added near `parseIsoUtcToLocal`. **r276** — DST root-cause fix. The old `parseIsoUtcToLocal` called `mktime()` three times as a throwaway kludge with `tm_isdst=0` fields, which mutated newlib's internal DST state; `localtime_r` then applied the DST savings hour a second time, producing a +3 h offset (UTC → "local+1"). Fixed by deleting all mktime calls and keeping only the manual Zeller-like days-since-epoch computation, then a single clean `localtime_r` that honors the configured `CET-1CEST,M3.5.0/2,M10.5.0/3` POSIX TZ correctly. Companion bug in `netFetchTransitDepartures` ([scrybar.ino:8767](../scrybar.ino:8767)): the `nowUtc` used for the `?time=` query parameter was computed via the same mktime dance and landed 1 h in the past during CEST, so the API returned departures already gone. Replaced the whole block with `const time_t nowUtc = time(nullptr);` — on ESP-IDF after `configTzTime()` NTP sync, `time(nullptr)` returns the true UTC epoch directly. Net diff: r275 +60/−12; r276 +14/−38.
- Impact/Tradeoffs: All transit-time rendering is now correct year-round under both CET and CEST. The prime-notation countdown matches the physical "cartello alla fermata" UX Lobo asked for. The PARSE diagnostic is one-line-per-row so serial monitor output stays readable; it fires on every fetch so data-quality bugs on any operator feed can be narrowed to {wrong-stopId, timezone-parse, stale-GTFS} by eye. The `time(nullptr)` fix also means the stoptimes query now asks the API for "departures from now on" instead of "from 1 h ago", so no more already-passed trains in the first rows. Reference screenshots: `knowledge/screenshots/timetable_r276_default.png` (scrybar-default), `knowledge/screenshots/timetable_r276_cathode-ray.png`. Verified 7 themes — all readable, prime glyph present in Latin-1 charset for every Funnel Display instance. Spec: `docs/superpowers/specs/2026-04-22-timetable-relative-time-design.md`.

---

## 2026-04-22 - Mac Stats page: companion IOKit feed + 9th firmware page (r263, r264)

- Context: Backlog item: "Companion: Mac stats screen — nuova pagina companion con temp CPU/GPU, RAM free, disco free. Via `host_statistics64` + IOKit SMC. Sync verso ScryBar come pagina dedicata." The companion already polled `api/now-playing` every 1s but had no channel for hardware telemetry. ScryBar had 8 pages (r262) with the archived ANSI page slot (`0x08`) unused.
- Decision: **Two-commit series.** r263 adds the full 9-site page implementation; r264 adds serial navigation commands. **Companion side** (`MacStatsProvider.swift`): new `MacStatsPayload` struct (`cpuTempC?`, `gpuTempC?`, `ramUsedGB/TotalGB`, `diskUsedGB/TotalGB`, `updatedAt`). `MacStatsProvider` samples RAM via `host_statistics64(HOST_VM_INFO64)` + `vm_statistics64_data_t` (free+speculative pages counted as available, inactive excluded from "used"), disk via `FileManager.attributesOfFileSystem(forPath: "/")`, temperatures via `SMCReader` (minimal 80-byte IOKit IPC, selector 2). `SMCReader` uses raw `Data` buffers with explicit byte offsets: key at offset 0 (FourCC big-endian), dataSize at offset 28 (LE), dataType at offset 32 (BE), command at offset 42, value bytes at offset 48. CPU keys tried in order: `TC0P/TC0D/TC0E/Tp09/Tp0P`. GPU keys: `TGDD/TG0D/TGOP/Tg05/Tg0f`. Temperature type `sp78` only (signed 8.8 fixed-point, 2 bytes). Swift 6 gotcha: `vm_page_size` is a global mutable — use `getpagesize()` instead. `AppModel` starts a parallel `macStatsPollTask` (every 5s, detached background Task for IOKit blocking calls, silent error suppression). `ScryBarClient.sendMacStats()` POSTs JSON to `api/mac-stats` with 5s timeout. **Firmware side:** `UI_PAGE_MAC_STATS=8` added to `UiPageMode` enum. `UI_VIEW_FLAG_MAC_STATS=0x08` reuses the archived ANSI page bit slot (safe — that slot was never included in `UI_VIEW_MASK_DEFAULT` before r263). Page gate: `flag AND g_wifiSt.connected` (mirrors LAUNCH pattern). `handleWebMacStatsPostApi()` uses hand-rolled `extractJsonNumberField()` (NOT ArduinoJson — firmware never uses that library). `lvglInitMacStatsUi()`: 2×2 tile grid, header 30px, content 610×142 divided into four 305×71 tiles — CPU tile (top-left) + GPU tile (top-right) with 32px bold values, RAM tile (bottom-left) + DISK tile (bottom-right) with 20px "X / Y GB" values and bar fill. Bar fills hardcoded `0x5599FF` blue (no `accentPrimary` field in `UiThemeLvglTokens`). `lvglUpdateMacStatsUi()`: temp color-coding (≥85°C red `0xFF5252`, ≥70°C amber `0xFFB74D`), disk bar color changes at ≥75% usage (amber) and ≥90% (red), age label, noData placeholder when `g_macStats.valid==false`. Serial commands (r264): `VIEW8`, `VIEWMACSTATS`, `MACSTATS`.
- Impact/Tradeoffs: The companion now sends two parallel data streams to ScryBar: media metadata (1s) and hardware telemetry (5s). The ANSI bit reuse means any future attempt to restore the ANSI page would need to pick a new bit or extend the mask to uint16_t. Bar accent color is hardcoded across all 7 themes (no token) — acceptable as a first pass; a future token (`accentBlue` or similar) could replace it without UI changes. Flash cost: ~2 KB for LVGL widget tree + parser. Verification: `[MACSTATS][API] cpu=58.2(ok=1) gpu=71.0(ok=1) ram=11.4/16.0 disk=329.0/500.0` confirmed on device; screenshot showed correct 2×2 layout, amber GPU value, and bar fills.

---

## 2026-04-21 - Web-UI dual brightness sliders + views wiring fix (r261, r262)

- Context: The firmware shipped with two hardcoded backlight levels (`100` on USB, `ENERGY_BACKLIGHT_ON_BATTERY=72` on battery) and no way to tune them without reflashing. The web-UI views section had two latent bugs: the **Departures** checkbox wrote `UI_VIEW_FLAG_TRANSIT` into `enabledViewsMask` but `uiPageEnabledNoEnsure(UI_PAGE_TRANSIT)` ignored the flag and returned only `g_transitConfig.configured` — so the toggle had zero effect. **Launches** (r252, scrybar.ino:931 `UI_PAGE_LAUNCH=7`) had never been added to `UI_VIEW_FLAG_*`, never shown in the web-UI, and auto-activated as soon as WiFi connected with no user control.
- Decision: Two commits. **r261** adds `RuntimeNetConfig.backlightUsbPct` / `backlightBatPct` (clamped 10..100, defaults 100/72 so existing devices see no change). `applyEnergyPolicy` now reads those fields. New `POST /api/brightness` endpoint accepts `usb=<int>&batt=<int>`, clamps silently (the handler fires during drag), writes to RAM only, and calls `applyEnergyPolicy(millis(), true)` to repaint immediately. The `force=true` path now skips the "mode unchanged → early return" to let live slider pushes propagate even when the power source hasn't flipped. Form save persists through the existing NVS path (`bl_usb` / `bl_batt` args). Web-UI gains a `vm-card` "Display Brightness" between Themes and Views with two `<input type=range>` backed by 120 ms debounced `fetch('/api/brightness')` on `oninput`. **r262** adds `UI_VIEW_FLAG_LAUNCH=0x80`, includes it and `UI_VIEW_FLAG_TRANSIT` in `UI_VIEW_MASK_DEFAULT`, and rewrites the two page-enabled gates as `flag AND functional_gate` (`g_transitConfig.configured` / `g_wifiSt.connected`) so the checkbox actually disables the page. The existing `ui_views_gen` NVS migration mechanism automatically adds both new flags as ON for devices upgraded from r261 or earlier. Spec: `docs/superpowers/specs/2026-04-21-web-ui-brightness-sliders.md`.
- Impact/Tradeoffs: Brightness is tunable cross-session, per power source, without a flash. Live endpoint is deliberately lossy (silently clamps, no NVS) — the form save is the source of truth. Minimum 10 % floor prevents accidental black-screen brick via drag. The views fix exposed a class of bug that the decisions doc now flags: adding a new `UI_PAGE_*` requires matching a new `UI_VIEW_FLAG_*`, `UI_VIEW_MASK_DEFAULT` bit, `uiViewFlagForPage` mapping, web-UI checkbox, parser entry, `webRequestHasConfigParams` sniffer, `kViewArgs` entry, JSON API field, and the JS `pairs` array. Net diff: r261 +124/−2, r262 +26/−7. Commits: `888cb95` (spec), `07e5d4d` (r261), `9acfa92` (r262).

---

## 2026-04-21 - Screensaver unified cross-theme redesign — Nordic Night + Stacked Strip + Balanced (r260)

- Context: Before r260 the Pasture Simulator screensaver pulled colors from each of the 7 UI themes via `UiThemeLvglTokens.saverSky/saverField/saverCow/saverBalloon/saverFooter/saverStarLow/saverStarMid/saverStarHigh`. In practice stars were near-invisible in some themes, the scrolling grass row sometimes overlapped the bottom-right clock, and maintenance cost was high (56 hand-tuned values plus five event systems and four cow states). User asked for a single unified identity cross-theme, with stars and grass always clearly visible and no overlap with the clock.
- Decision: Three-axis unification validated in a companion brainstorming session — (1) **Palette Nordic Night** (`kSaverPaletteBg=0x0A0F1E` indigo, `kSaverPaletteStarHi/Mid/Lo=0xFFFFFF/0xB8D0FF/0x4A5878`, `kSaverPaletteCow=0xE8EEFF` cream, `kSaverPaletteGrass=0x2F5940` teal, `kSaverPaletteBalloon=0x9FB0D6`, `kSaverPaletteClock=0xC8D6FF`) hardcoded, theme-independent; (2) **Layout Stacked Strip** — four fixed vertical zones (sky 0..72 / cow 72..132 / grass 132..150 / clock 150..172) exposed as `kSaverZoneSkyTop/Bot/CowTop/Bot/GrassTop/Bot/ClockTop` constants; sky uses exactly 6 rows at 12-px pitch starting at y=4, covering 72 px of height; cow anchored at (20, cH-90) so its 3 rows sit inside 82..118; grass label is one row at (4, 132) sized (cW-8, 18); clock aligned bottom-right with existing anti-burn-in jitter; (3) **Balanced feature scope** — keep stars (3-layer flicker), grass (scroll + `/^\|` drifting tree), cow states GRAZE / IDLE / SLEEP with bias toward SLEEP (55/25/20), balloon with only `THOUGHT_PHILOSOPHY` (70%) and `THOUGHT_EASTER_EGG` (30%), events only `SAVER_EVENT_SHOOTING_STAR` and `SAVER_EVENT_SATELLITE`; park `COW_RUN` / `COW_STARE_UP`, balloon cats `HACKER` / `META` / `WEATHER`, events `UFO` / `RAIN` / `MATRIX_GLITCH`, and sky phase transitions (sky forced to `SKY_NIGHT`). The function `lvglResolvedSaverReadableText` is removed — no longer needed once balloon/footer use hardcoded constants. The `UiThemeLvglTokens.saverXxx` fields remain on every theme definition for ABI stability but are no longer read by the saver code path. Spec: `docs/superpowers/specs/2026-04-21-screensaver-unified-redesign.md`. Deferred-features memory: `memory/screensaver_deferred_features.md`. Reference snapshot: `knowledge/screenshots/saver_unified_r260.png`.
- Impact/Tradeoffs: Flash footprint dropped ~3.5 KB (unused color resolution, fewer event/state code paths). Zero user-facing behavior change for any code path outside the screensaver — the unified look activates after the normal 2-hour idle threshold or `SAVERON` command. Rollback is a single revert of the r260 commit because the per-theme saver color fields stay intact. Next session can enable any parked feature by lifting the gate at its individual site (event roll, cow transition table, balloon category roll) without re-introducing the whole Rich-tier bundle. Known non-goals: the screensaver activation threshold is unchanged; there is no web-UI toggle yet; the cow ASCII art is not redesigned; the 7-language balloon pool is preserved intact. Verification on device: `SAVERON` serial command + `tools/capture_snapshot.py` confirmed all four zones render correctly with the Nordic Night palette on the current theme (Cathode Ray) — no grass/clock overlap, all three star brightness levels visible, cow art centered in its band.

---

## 2026-04-21 - uint32_t `millis()` cross-handler race — the rapid-tap-wakes-screensaver bug (r257→r259)

- Context: User reported that rapid-tapping the center of the WIKI page would often "sneak in" the Pasture screensaver — with cow, stars and quotes, the real thing — even though the idle threshold is 2 hours and the device had just been actively used. Slow tapping never triggered it. Hypotheses we tried and discarded before finding the real cause: (A) a race in the `awaitRelease` wake gate — inconsistent with "saver remains active"; (B) capacitive noise on `PWR_BUTTON_PIN` (GPIO16) inducing a fake short-press via `onPowerButtonShortPress` — superficially plausible, motivated a first defensive fix in r257 (PWR-ignore-while-touch), but serial logs later proved this was wrong. The diagnostic log `[SCRNSVR] ON reason=idle idleMs=4294967294 targetMs=7200000` made the real cause obvious: `idleMs = 0xFFFFFFFE` is a uint32_t subtraction underflow.
- Root cause: `loop()` at [scrybar.ino:17985](scrybar.ino:17985) captures `const uint32_t now = millis();` once at the top and passes the same `now` to every handler. Handlers called *later* in the same iteration — `handleTouchSwipeInput` ([scrybar.ino:11864](scrybar.ino:11864) reads its own fresh `millis()`), `runImuLoop` ([scrybar.ino:17830](scrybar.ino:17830) ditto), and the *async* WiFi event task `onWiFiEvent` ([scrybar.ino:3264](scrybar.ino:3264)) — each stamp timestamp fields (`g_saver.lastUserInteractionMs`, `g_imu.lastShakeMs`, `g_wifiSt.lastDisconnectMs`, `g_wifiSt.noLinkSinceMs`, `g_liveNowPlaying.receivedAtMs`) with those fresher reads. Then downstream handlers like `handleScreenSaverLoop(now)` use the stale loop snapshot. When `storedMs > now` by even 1 ms, the `uint32_t` subtraction `(now - storedMs)` wraps to ~4.29×10⁹, which is ≥ any threshold, so every idle/debounce check fires falsely. Rapid tapping is the trigger because it's the only user action that causes `lastUserInteractionMs` to be stamped with high-frequency fresh timestamps that can land inside the same loop iteration as `handleScreenSaverLoop`.
- Decision: Fix at the root by refreshing `nowMs = millis();` at entry of every handler that receives the loop's stale `now` AND compares it against fields stampable elsewhere. Applied to `handleScreenSaverLoop` (covers the 2h-idle check AND the 1.2s shake-wake check in a single line) and `wifiHandleSetupModeLoop`. For one-off call sites against async-stamped fields where a full refresh is overkill, use the clamp pattern `const uint32_t delta = (now >= stored) ? (now - stored) : 0;` — applied at `wifiSetupApActiveRecently` ([scrybar.ino:13457](scrybar.ino:13457)) for the 20 s post-disconnect window. Also kept a belt-and-suspenders clamp inside the saver idle branch for defense-in-depth, and added `[SCRNSVR] ON reason=<idle|pwr_short_press|cmd>` log lines so any future spurious activation is immediately attributable. Reverted the wrong r257 PWR-ignore guard in `onPowerButtonShortPress` — harmless but based on a false hypothesis. Documented the full rule in `knowledge/project_knowledge.md` "Recurring Gotchas".
- Impact/Tradeoffs: Fix cost is ~15 µs per loop iteration (one extra `millis()` call per refreshed handler) — unmeasurable. The rule is simple, mechanical, and auditable: any `loop()`-fed handler that does `(nowMs - storedMs)` where `storedMs` can be stamped by another handler must either refresh `nowMs` at entry (preferred, one-line) or clamp each subtraction (for single-site async-stamped fields). Investigation cost: ~90 minutes across a wrong hypothesis (PWR noise), two flash cycles (r257 diagnostic + r258 real fix + r259 full sweep), and a codebase-wide audit of every `(now - *Ms)` subtraction in `scrybar.ino`. The gotcha is now entry #16 in the Recurring Gotchas section and flagged as CRITICAL — this class of bug is easy to miss at code review and silent until the race window aligns with real user input. Build sequence: r257 (wrong fix, still repro'd), r258 (correct clamp at idle site, manual verify clean), r259 (structural refresh across all 3 affected handlers). Test protocol: tap-spam WIKI for 30 s; no `[SCRNSVR] ON reason=idle` events in serial log.
